...
import { X, Save, Hotel as HotelIcon, MapPin, Wallet, Image as ImageIcon, Star, Upload, Loader2, Sparkles } from 'lucide-react';
...
export default function HotelForm({ hotel, onClose }: HotelFormProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('Starting hotel image upload for file:', file.name, 'size:', file.size);
    try {
      const storageRef = ref(storage, `hotels/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9.]/g, '_')}`);
      ...
      const downloadURL = await new Promise<string>((resolve, reject) => {
        ...
        uploadTask.on('state_changed', 
          (snapshot) => {
            ...
            console.log('Hotel upload is ' + progress + '% done', 'state:', snapshot.state);
          }, 
          (error) => {
            // clearTimeout(timeout);
            console.error('Firebase Storage hotel upload error (full):', error);
            reject(error);
          }, 
          async () => {
            // clearTimeout(timeout);
            console.log('Hotel upload completed successfully');
  ...
  const handleGalleryImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('Starting hotel gallery image upload for file:', file.name, 'size:', file.size);
    try {
      const storageRef = ref(storage, `hotels/gallery/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9.]/g, '_')}`);
      const uploadTask = uploadBytesResumable(storageRef, file);
      ...
      const downloadURL = await new Promise<string>((resolve, reject) => {
        ...
        uploadTask.on('state_changed', 
          (snapshot) => {
            const progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
            console.log('Hotel gallery upload is ' + progress + '% done', 'state:', snapshot.state);
          }, 
          (error) => {
            // clearTimeout(timeout);
            console.error('Firebase Storage hotel gallery upload error (full):', error);
            reject(error);
          }, 
          async () => {
            // clearTimeout(timeout);
            console.log('Hotel gallery upload completed successfully');
            const url = await getDownloadURL(uploadTask.snapshot.ref);
            resolve(url);
          }
        );
  ...
  const handleAIGenerate = async () => {
    if (!formData.name) {
      alert('Vui lòng nhập tên khách sạn trước!');
      return;
    }

    setIsGenerating(true);
    try {
      const details = await generateLocationDetails(formData.name, 'hotel', formData.city || 'shanghai');
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hotel) {
        await updateDoc(doc(db, 'hotels', hotel.id), cleanData);
      } else {
        await addDoc(collection(db, 'hotels'), cleanData);
      }
      onClose();
    } catch (error) {
      handleFirestoreError(error, hotel ? OperationType.UPDATE : OperationType.CREATE, 'hotels');
    }
  };

  return (
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl border border-stone-100">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-orange-100 text-orange-600 rounded-full text-xs font-bold hover:bg-orange-200 transition-all disabled:opacity-50"
                >
                  {isGenerating ? (
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-stone-400">Thông tin lưu trú</p>
            </div>
          </div>
          <button onClick={onClose} className="p-3 hover:bg-white rounded-2xl transition-all border border-transparent hover:border-stone-200">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-stone-400 flex items-center gap-2">
                <HotelIcon className="w-3 h-3" /> Tên khách sạn
              </label>
              <input
                required
                type="text"
                ...
                placeholder="VD: The Peninsula Shanghai"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-stone-400 flex items-center gap-2">
                <Wallet className="w-3 h-3" /> Giá mỗi đêm
              </label>
              <input
                required
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-stone-400 flex items-center gap-2">
                <MapPin className="w-3 h-3" /> Khu vực / Địa chỉ
              </label>
              <input
                required
                type="text"
                ...
                placeholder="VD: Bến Thượng Hải, Hoàng Phố"
                className="w-full px-4 py-3 bg-stone-50 border border-stone-100 rounded-2xl focus:ring-2 focus:ring-orange-500 outline-none transition-all"
              />
            </div>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              {formData.imageUrl && (
                <div className="relative w-32 h-32 rounded-2xl overflow-hidden border border-stone-100 mt-2">
          ...
          <div className="flex items-center gap-3 p-4 bg-orange-50 rounded-2xl border border-orange-100">
            <input
              type="checkbox"
              id="isNearCenter"
              checked={formData.isNearCenter}
              onChange={(e) => setFormData({ ...formData, isNearCenter: e.target.checked })}
              className="w-5 h-5 rounded border-orange-300 text-orange-600 focus:ring-orange-500"
            />
            <label htmlFor="isNearCenter" className="text-sm font-bold text-orange-900 cursor-pointer flex items-center gap-2">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Mô tả về tiện ích, phong cách khách sạn..."
          ...
          <div className="flex gap-4 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-8 py-4 bg-stone-100 text-stone-600 rounded-2xl font-bold hover:bg-stone-200 transition-all"
            >
              Hủy
            </button>
            <button
              type="submit"
              className="flex-2 px-8 py-4 bg-stone-900 text-white rounded-2xl font-bold hover:bg-stone-800 transition-all flex items-center justify-center gap-2 shadow-lg shadow-stone-200"
            >
              <Save className="w-5 h-5" />
              {hotel ? 'Cập nhật' : 'Lưu khách sạn'}
            </button>
          </div>
        </form>
      </div>
